...
const ChangePassword: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setServerError({});
      (
        document.getElementById("password-change-form") as HTMLFormElement
      ).reset();
    } catch (error) {
      ...
      >
        ...
        >
          ...
          {server_error.password && (
            <Typography className="text-red-500 text-sm pl-2">
          ...
          <Box className="text-center mt-8">
            <Button type="submit" variant="contained" className="px-5">
              {" "}
              Update{" "}
            </Button>
          </Box>
